...
int display_check(void) {
  ...
  if (framebuffer_request.response->framebuffers[0]->width > DISPLAY_WIDTH ||
      framebuffer_request.response->framebuffers[0]->height > DISPLAY_HEIGHT) {
    return 0;
  }
...
void put_char(char symbol) {
  ...
  // Assume that each char is 16x32px. May not always be correct but for now
  // that's what I'm using After putting a char lets go to the next cell
  cur_x += 16;
